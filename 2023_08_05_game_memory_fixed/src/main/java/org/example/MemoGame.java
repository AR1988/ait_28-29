...
public class MemoGame {
    ...
    public static void main(String[] args) {
        ...
        int boardHigh, boardWeight;
        do {
            System.out.println("Введите размер поля:");
            System.out.println("Высота");
            boardHigh = scanner.nextInt();
            System.out.println("Ширина");
            boardWeight = scanner.nextInt();
            isValid = BoardService.validateDimension(boardHigh, boardWeight);
            if (!isValid) {
                System.out.println("Введите четное значение ширины и длины");
            }
        } while (!isValid);

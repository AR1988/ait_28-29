...
public class TransactionParser implements Parser<Transaction> {
    ...
    public Transaction parse(String line) {
        ...
        return new Transaction(Long.valueOf(split[0]),
                LocalDateTime.parse(split[1], CSV_DATA_TIME_FORMATTER),
                BigDecimal.valueOf(Double.parseDouble(split[2])),
                subCategoryService.getById(Long.parseLong(split[3])));
    }
}
